...
class TokenPaymentsViewModel {
    ...
    fun getJudo(widgetType: PaymentWidgetType): Judo = with(initialJudoConfig) {
    ...
    private fun getCardToken() =
        CardToken(lastFour = lastFour, token = token, type = scheme.toInt(), cardHolderName = cardholderName)
}
