...
internal fun CardPattern.colorRes(context: Context) =
    when (this) {
...
internal fun CardPattern.drawableRes(context: Context) =
    when (this) {
...
internal fun CardPattern.cardIcon(cardNetwork: CardNetwork): Int =
    when (cardNetwork) {
        CardNetwork.VISA ->
            when (this) {
                CardPattern.BLACK,
                CardPattern.TWILIGHT_BLUE,
                -> R.drawable.ic_card_visa_light
